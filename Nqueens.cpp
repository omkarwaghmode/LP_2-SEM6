...
void prnt(vector<vector<int>> board)
{
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
...
bool isafe(int row, int col, vector<bool> rows, vector<bool> ld, vector<bool> rd)
{
...
bool solve(vector<vector<int>> &board, int col, vector<bool> rows, vector<bool> ld, vector<bool> rd)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (isafe(i, col, rows, ld, rd) == true)
        {
            rows[i] = true;
            ld[i + col] = true;
            rd[col - i + n - 1] = true;
            ...
            if (solve(board, col + 1, rows, ld, rd) == true)
            {
                return true;
            }
